...
// 若已存在则返回 model.ErrDuplicate，不覆盖已有数据。
func (s *Store) InsertSample(p *model.SamplePoint) error {
	res, err := s.db.Exec(
		`INSERT OR REPLACE INTO samples (batch_id, channel_id, seq, t_unix, value, created_at)
		 VALUES (?, ?, ?, ?, ?, ?)`,
...
// InsertSamplesTx 在事务内批量写入采样点，任一重复即整体回滚（幂等整段）。
